...
%% Efficiency Metrics
% Stage efficiencies - calculate as output/input power ratio
% Ensure we don't divide by zero and handle edge cases

% Sprocket: output power / input power
if P_mechanical_drum > 1e-6
    P_sprocket_out = max(0, P_mechanical_drum - P_loss_sprocket);
    eta_sprocket = P_sprocket_out / P_mechanical_drum;
else
    eta_sprocket = 0;
end
eta_sprocket = max(0, min(1, eta_sprocket));  % Clamp to [0, 1]

% Gearbox: output power / input power
P_gearbox_in = max(0, P_mechanical_drum - P_loss_sprocket);
if P_gearbox_in > 1e-6
    P_gearbox_out = max(0, P_gearbox_in - P_loss_gearbox);
    eta_gearbox = P_gearbox_out / P_gearbox_in;
else
    eta_gearbox = 0;
end
eta_gearbox = max(0, min(1, eta_gearbox));  % Clamp to [0, 1]

% Bevel: output power / input power
P_bevel_in = max(0, P_mechanical_drum - P_loss_sprocket - P_loss_gearbox);
if P_bevel_in > 1e-6
    P_bevel_out = max(0, P_bevel_in - P_loss_bevel);
    eta_bevel = P_bevel_out / P_bevel_in;
else
    eta_bevel = 0;
end
eta_bevel = max(0, min(1, eta_bevel));  % Clamp to [0, 1]

% Motor: electrical power out / mechanical power in
P_motor_mech_in = max(0, P_mechanical_drum - P_loss_mechanical);
if P_motor_mech_in > 1e-6
    eta_motor = max(0, P_electrical_instantaneous) / P_motor_mech_in;
else
    eta_motor = 0;
end
eta_motor = max(0, min(1, eta_motor));  % Clamp to [0, 1]

% Overall system efficiency: electrical power / mechanical input power
if P_mechanical_drum > 1e-6
    eta_system = max(0, P_electrical_instantaneous) / P_mechanical_drum;
else
    eta_system = 0;
end
eta_system = max(0, min(1, eta_system));  % Clamp to [0, 1]
