...
% Total power loss
% Should equal: efficiency loss + friction loss
P_loss_calc = P_in - (T_out * omega_out);
P_loss_expected = P_in * (1 - eta_total) + P_loss_friction;
% Use the more accurate calculation, but ensure it's non-negative
P_loss = max(0, P_loss_calc);
% Sanity check: loss should be reasonable
if P_loss > P_in * 1.1  % More than 110% of input is impossible
    P_loss = P_loss_expected;  % Fall back to expected value
end
