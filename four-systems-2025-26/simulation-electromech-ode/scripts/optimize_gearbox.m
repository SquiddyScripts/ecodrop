...
%% Find Optimal Ratio
% Filter out results with energy conservation errors > 5%
energy_error_threshold = 0.05;  % 5%
valid_energy_mask = true(size(ratios_valid));
for i = 1:length(ratios_valid)
    % Find corresponding index in full results_array
    ratio_val = ratios_valid(i);
    array_idx = find([results_array.ratio] == ratio_val, 1);
    if ~isempty(array_idx) && ~isempty(results_array(array_idx).simulation)
        if isfield(results_array(array_idx).simulation, 'validation')
            if isfield(results_array(array_idx).simulation.validation, 'energy_conservation')
                error_pct = results_array(array_idx).simulation.validation.energy_conservation.error_percent / 100;
                if error_pct > energy_error_threshold
                    valid_energy_mask(i) = false;
                end
            end
        end
    end
...
% Apply energy conservation filter
ratios_final = ratios_valid(valid_energy_mask);
E_electrical_final = E_electrical_valid(valid_energy_mask);
efficiencies_final = efficiencies_valid(valid_energy_mask);
drop_times_final = drop_times_valid(valid_energy_mask);

if isempty(ratios_final)
    warning('No valid results found after filtering. Using all valid results.');
    ratios_final = ratios_valid;
    E_electrical_final = E_electrical_valid;
    efficiencies_final = efficiencies_valid;
    drop_times_final = drop_times_valid;
    valid_energy_mask = true(size(ratios_valid));
end

switch lower(metric)
    case 'efficiency'
        [opt_value, opt_idx_local] = max(efficiencies_final);
        opt_ratio = ratios_final(opt_idx_local);
        % Find original index
        opt_idx = find(ratios_valid == opt_ratio & valid_energy_mask, 1);
    case 'energy'
        [opt_value, opt_idx_local] = max(E_electrical_final);
        opt_ratio = ratios_final(opt_idx_local);
        opt_idx = find(ratios_valid == opt_ratio & valid_energy_mask, 1);
    case 'power'
        % Power = Energy / Time
        power = E_electrical_final ./ drop_times_final;
        [opt_value, opt_idx_local] = max(power);
        opt_ratio = ratios_final(opt_idx_local);
        opt_idx = find(ratios_valid == opt_ratio & valid_energy_mask, 1);
    otherwise
