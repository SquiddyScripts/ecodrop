...
if include_electrical_dynamics && length(state) >= 4
    ...
    % Quasi-static: calculate current from load model iteratively
    % Need to solve: V_terminal = V_emf - I*R, and I = load_model(V_terminal)
    % Use iterative solution (simple fixed-point iteration)
    V_emf = params.motor.K_e * omega_motor;
    I_motor = 0;  % Initial guess
    max_iter = 10;
    tol = 1e-6;
    for iter = 1:max_iter
        V_terminal_old = V_emf - I_motor * params.motor.R_winding;
        I_motor_new = load_model(V_terminal_old, t, params);
        if abs(I_motor_new - I_motor) < tol
            I_motor = I_motor_new;
            break;
        end
        I_motor = I_motor_new;
    end
    % Final current limit check
    I_motor = min(I_motor, params.motor.I_max);
end
...
% Recalculate load current with correct terminal voltage (iterative refinement)
if ~include_electrical_dynamics
    % Refine current calculation with actual terminal voltage
    I_motor_new = load_model(V_terminal, t, params);
    I_motor_new = min(I_motor_new, params.motor.I_max);  % Enforce limit
    if abs(I_motor_new - I_motor) > 1e-3  % If significant change, recalculate
        I_motor = I_motor_new;
        [T_motor, V_emf, V_terminal, P_electrical, P_losses_motor] = ...
            motor_model(omega_motor, I_motor, params);
    end
