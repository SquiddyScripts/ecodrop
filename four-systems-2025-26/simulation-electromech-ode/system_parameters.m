...
%% Physical System Parameters (tuned for clear, impressive curves)
params.physical = struct();
params.physical.mass = 15.0;              % Mass of falling object [kg]
params.physical.g = 9.81;                 % Gravitational acceleration [m/s²]
params.physical.drum_radius = 0.06;      % Drum radius [m]
params.physical.drum_inertia = 0.012;    % Drum moment of inertia [kg·m²]
params.physical.sprocket_ratio = 1.2;    % Sprocket speed ratio (output/input)
params.physical.sprocket_inertia = 0.005; % Sprocket moment of inertia [kg·m²]
params.physical.initial_height = 3.0;    % Initial drop height [m]
params.physical.initial_velocity = 0.0;   % Initial velocity [m/s]

%% Gearbox Parameters
params.gearbox = struct();
params.gearbox.ratios = [4.0];            % Gearbox ratios (smooth power curve, good efficiency)
...
params.electrical.R_load = 0.15;           % Load resistance [Ω] (resistive load – good power/voltage balance)
...
params.optimization.gearbox_ratio_min = 0.5;   % Minimum gearbox ratio (reduced to find better optimum)
params.optimization.gearbox_ratio_max = 20.0; % Maximum gearbox ratio (reduced from 50.0 for faster optimization)
