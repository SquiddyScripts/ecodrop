...
% Linear kinetic energy of mass
E_kin_linear = 0.5 * params.physical.mass * v_linear(end)^2;

% Rotational kinetic energy - calculate directly from final state
omega_mass_final = omega_mass(end);
omega_motor_final = omega_motor(end);

% Drum inertia (without mass - mass is accounted separately as linear KE)
E_kin_drum = 0.5 * params.physical.drum_inertia * omega_mass_final^2;

% Motor rotor
E_kin_motor = 0.5 * params.motor.J_rotor * omega_motor_final^2;

% Sprocket, gearbox, bevel (approximate - reflected inertias)
N_sprocket = params.physical.sprocket_ratio;
N_gearbox = prod(params.gearbox.ratios);
N_bevel = params.bevel.ratio;

omega_sprocket = omega_mass_final / N_sprocket;
omega_gearbox_out = omega_mass_final / (N_sprocket * N_gearbox);

E_kin_sprocket = 0.5 * params.physical.sprocket_inertia * omega_sprocket^2;
E_kin_gearbox = 0.5 * (params.gearbox.inertia_input + params.gearbox.inertia_output) * omega_gearbox_out^2;
E_kin_bevel = 0.5 * params.bevel.inertia * omega_motor_final^2;

E_kin_rotational = E_kin_drum + E_kin_sprocket + E_kin_gearbox + E_kin_bevel + E_kin_motor;

E_kinetic_final = E_kin_linear + E_kin_rotational;
...
fprintf('  Final kinetic energy (linear + rotational): %.2f J\n', E_kinetic_final);
fprintf('    - Linear (mass): %.2f J\n', E_kin_linear);
fprintf('    - Rotational (all components): %.2f J\n', E_kin_rotational);
